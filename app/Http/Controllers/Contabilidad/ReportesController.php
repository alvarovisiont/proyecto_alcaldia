...
class ReportesController extends Controller
{
    ...
    {
      return view('contabilidad.reportes.balance_general');
    }

    public function balance_general_pdf(Request $request)
    {
        $hasta = date('Y-m-d',strtotime($request->get('hasta')));
      $año   = date('Y',strtotime($request->get('hasta')));
      $desde = $año.'-01-01';
      ...
      $balance = DB::select($sql);
      $config  = Cont_config::where('status','=',1)->first(); 

      $datos = ['balance' => $balance, 'hasta' => $request->get('hasta'),'total_activo_1' => 0,'total_activo_2' => 0,'total_activo_3' => 0,'total_pasivo_1' => 0,'total_pasivo_2' => 0,'total_pasivo_3' => 0,'config' => $config];


      $pdf = PDF::loadView('contabilidad.reportes.pdf.balance_general_pdf',$datos);
