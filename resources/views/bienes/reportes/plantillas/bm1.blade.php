	...
	<body>
		...
		<table width="100%">
			<tr>
				...
				<td  align="right"><strong>Formulario del BM-1</strong></td>
			</tr>
		</table>
		<hr>
		<h3 style="text-align: center">INVENTARIO DE BIENES MUEBLES</h3>
		<br>
		<table width="100%">
			<tr>
		...
		<table class="tablep">
			...
			<tbody class="centrado">';
				@foreach($bienes AS $row)
					<tr>
						<td>{{$row->unidad->id}}</td>
						<td>{{$row->seccion->id}}</td>
						<td>{{$row->tipo->nomenclatura->grupo}}</td>
						<td>{{$row->tipo->nomenclatura->sub_grupo}}</td>
						<td>{{$row->codigo}}</td>
